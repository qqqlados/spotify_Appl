...
const ArtistRelatedArtists = ({ artist_id }: { artist_id: string | undefined }) => {
	const { relatedArtists } = useRelatedArtists(artist_id!)
	...
	return (
		<div className={s.related_artists}>
			...
			<div className={s.content}>{isLoading ? <LoaderCircle /> : isError ? <ErrorMessage /> : <ArtistsList artists={relatedArtists} />}</div>
		</div>
	)
}
