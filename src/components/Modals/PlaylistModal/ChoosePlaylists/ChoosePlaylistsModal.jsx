...
const ChoosePlaylistsModal = ({ setModalOptions, setChoosePlaylistsModal, trackUri, trackId }) => {
	...
	const { checkPlaylistTracks, isLoadingAddTrack, isAddTrackSuccess, isAddTrackError } = useAddTrack(playlistId, trackId, trackUri)
	...
	useEffect(() => {
		if (isAddTrackSuccess && !isLoadingAddTrack) {
			setChoosePlaylistsModal(false)
		}
	}, [isAddTrackSuccess, isLoadingAddTrack])
			...
			<ul className={s.list}>
				{filteredPlaylists?.map((item, index) => (
					<li className={s.item} onClick={() => setPlaylistId(item?.id)} key={item.id}>
						...
						{isLoadingAddTrack && <LoaderCircle small={true} />}
					</li>
				))}
			</ul>
